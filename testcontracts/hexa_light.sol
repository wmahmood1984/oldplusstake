...
contract Helper is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function stakeAndBurn(address _user) public {
        ...
        for (uint i = 0; i < burntNFTs.length; i++) {
            if (burntNFTs[i]._owner == _user) {
                uint id = i;//burntNFTs[i].id;

                require(
                    id > 0,
                    string(
                        abi.encodePacked(
                            "REMOVE_USED: id underflow at burntNFTs index ",
                            _toString(i),
                            ", id=0"
                        )
                    )
                ...
                require(
                    index < burntNFTs.length,
                    string(
                        abi.encodePacked(
                            "REMOVE_USED: index out of bounds, index=",
                            _toString(index),
                            ", length=",
                            _toString(burntNFTs.length)
                        )
                    )
                );
        ...
        for (uint i = 0; i < NFTQueLocal.length; i++) {
            if (NFTQueLocal[i].user == _user) {
                require(
                    i < NFTQue.length,
                    string(
                        abi.encodePacked(
                            "REMOVE_QUEUE: index out of bounds, i=",
                            _toString(i),
                            ", length=",
                            _toString(NFTQue.length)
                        )
                    )
                );

                removeFirst(i);
                break; // IMPORTANT – avoid shifting + re-loop corruption
            }
        }
    }
